# Fine-tune MiniCPM 5 with LLaMA-Factory

> [LLaMA-Factory](https://github.com/hiyouga/LLaMA-Factory) is the most-used community fine-tuning framework. **MiniCPM 5 is a vanilla `LlamaForCausalLM`**, so it works out-of-the-box with LLaMA-Factory's standard recipe — no model-code patch, no template registry hack.
...
LLaMA-Factory consumes a `dataset_info.json` registry. For the MiniCPM 5 chat template, use `formatting: sharegpt` and the standard role tags:
...
template: empty           # MiniCPM 5 chat template auto-loads from the model's tokenizer_config.json
cutoff_len: 4096
max_samples: 100000
...
> 💡 `template: empty` tells LLaMA-Factory to **delegate to the tokenizer's built-in `chat_template.jinja`**, which is the MiniCPM 5 ChatML-style template (with Think / No-Think / tools support). Do **not** set `template: llama3` or other built-ins — they will produce a broken token layout.
...
Loss decreases monotonically — the framework + chat template + tokenizer combo all work end-to-end.
...
The MiniCPM 5 upstream repo ships single-page cookbooks for additional frameworks — pick the one that matches your workflow:

| Framework | Notes | Upstream cookbook |
| :--- | :--- | :--- |
| **TRL + PEFT** | Bare-metal Python recipe with assistant-only loss mask via `{% generation %}` | [`trl.md`](https://github.com/OpenBMB/MiniCPM/blob/minicpm5/docs/finetune/trl.md) |
| **ms-swift** | ModelScope toolkit; **must** pass `--model_type llama --template chatml` | [`ms_swift.md`](https://github.com/OpenBMB/MiniCPM/blob/minicpm5/docs/finetune/ms_swift.md) |
| **unsloth** | Single-GPU LoRA / QLoRA with custom kernels | [`unsloth.md`](https://github.com/OpenBMB/MiniCPM/blob/minicpm5/docs/finetune/unsloth.md) |
| **xtuner** | InternLM mmengine-based; use `PROMPT_TEMPLATE.qwen_chat` (= ChatML) | [`xtuner.md`](https://github.com/OpenBMB/MiniCPM/blob/minicpm5/docs/finetune/xtuner.md) |
...
LLaMA-Factory's `examples/deepspeed/` folder ships ZeRO-2 / ZeRO-3 / ZeRO-3-offload templates that all work directly with MiniCPM 5.
